...
class MultiHeadAttention_Enc_Dec(nn.Module):
    ...
    def forward(self, X, mask: Tensor = None) -> Tensor:
        x_sour, x_tar = X[0], X[1]  # enc is target, dec is source
        queries = rearrange(self.queries(x_sour), "b n (h d) -> b h n d", h=self.num_heads)
        keys = rearrange(self.keys(x_sour), "b n (h d) -> b h n d", h=self.num_heads)
        values = rearrange(self.values(x_tar), "b n (h d) -> b h n d", h=self.num_heads)
...
class ResidualAdd_Dec(nn.Module):
...
class DATrans():
    ...
    def train(self, X_train, y_train, X_val, y_val, X_test, y_test):
        ...
        self.Channels_Transformer.apply(weights_init_normal)
        # 获取数据
        sour_img = X_train
        sour_label = y_train
        ...
        self.optimizer_chan = torch.optim.Adam(self.channels_transformer.parameters(), lr=self.lr, betas=(self.b1, self.b2))
        averAcc = 0
        best_acc_val = 0
        ...
        for e in range(5):
            ...
            for i, (img, label, sour_img, sour_label) in enumerate(self.dataloader):
                ...
                if (i + 1) % 1 == 0:
                    ...
                    with torch.no_grad():
                        (feature, sour_feature) = self.Feature_Extractor((sour_img, img))
                    # discriminator
